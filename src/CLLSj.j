...
{ ASTType l,r,t; Token s;}
{
(   s=<Id>  { t = new ASTIdT(s.image); } ( <LPAR> ArgListT((ASTIdT)t) <RPAR> )?
    |
    <LINT> {
    t = new ASTLintT();
    }
...
(
    ...
    <LBRA> p=Proc() <RBRA>
    |
    c1=<Id> { p = new ASTId(c1.image); p.lineno = c1.beginLine;}
    (
    ((<COL> t=Type())? <LEFTA> c3=<Id> <DOT> c2=<lab>  <SEMIC> p2=Proc()  {

	   p = new ASTCoSSelect(c1.image,t, c3.image, c2.image,p2); // y:T <- x.#l; P
    })
    |
    (<DOT> c2=<lab> (<COL> t=Type())? <LEFTA>
    	   // <LPAR>
    	   (LOOKAHEAD(2)
     	   <LBRA> c3=<Id> <DOT> p1=Proc()  <RBRA> <SEMIC> p2=Proc()
    	   {
    ...
    { p = p1; p.lineno = c1.beginLine; }
    |
    c2 = <LETC>  c1=<Id> { t = null; } (<COL> t=Type())? <LBRA> p1=Proc() <RBRA>
    <LBRA> p2=Proc() <RBRA>
...
//    |
    ...
    |
    c2 = <CLOSE> c1=<Id>  { p = new ASTClose(c1.image); p.lineno = c2.beginLine; }
    |
