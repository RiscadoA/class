...
public class EnvBulk<X> { // static environment
    ...
    public EnvBulk(String _id, X _assoc, EnvBulk<X> _anc) {
	id = _id;
	assoc = _assoc;
	anc = _anc;
    }

    public EnvBulk(String _id, X _assoc, EnvBulk<X> _assocs, EnvBulk<X> _anc, boolean _moved){
    	id =_id;
    	assoc = _assoc;
    	assocs = _assocs;
    	anc = _anc;
    ...
    synchronized public EnvBulk<X> assoc(String id, X assoc) {
	return new EnvBulk<X>(id,assoc,this);
    }
    
    synchronized public void  insert(String _id, X _assoc) {
	if (assocs == null) assocs = new EnvBulk<X>();
	assocs = assocs.assoc(_id,_assoc);
    }

    synchronized public EnvBulk<X> shallowDup(){
	return new EnvBulk<X>(id, assoc, assocs, anc, moved);
    }

    synchronized public EnvBulk<X> dup() {
	EnvBulk<X>  de= new EnvBulk<X>(id,assoc,(anc==null)?null:anc.dup());
	return de;
    }

    synchronized public EnvBulk<X> dupe() {
	EnvBulk<X>  de= new EnvBulk<X>(id,assoc,(anc==null)?null:anc.dup());
	if (assocs != null) {
    ...
    synchronized public boolean eq(EnvBulk<X> e0) {
    ...
    synchronized public void updmove(EnvBulk<X> env2) throws Exception {
	if (moved) {
	    env2.upd(id,null);
	    moved = false;
	}
	...
	if (assoc == null) System.out.println("NULL");
		...
		if (assoc instanceof TypeEntry ) {
			...
			try {
			    System.out.println(tn.toStr((EnvBulk<EnvEntry>)this));
			}
			catch (Exception _e) { System.out.println(" ?X? ");  };
		    } else
			Pderef(assoc);
