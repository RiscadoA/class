...
public class ASTUnfold extends ASTNode {
  ...
  public void typecheck(Env<ASTType> ed, Env<ASTType> eg, Env<EnvEntry> ep) throws Exception {
    ...
    if (ty instanceof ASTRecT) {
      ...
      throw new TypeError(
          "Line " + lineno + " :" + "UNFOLD: " + ch + " is not of REC or COREC type " + ty);
  }
  ...
  public void samLC(
      String id, Channel channel, Env<SessionField> frame, Env<EnvEntry> ep, SAMCont p_cont)
      throws Exception {
    if (rec) { // REC
      if (CLLSj.trace) {
      ...
      if (rec) { // REC
        ...
        if (pol) {
          ...
          if (tyrhs.isPos(ep)) {
            ...
            // System.out.println("switchto pol = "+pold + " pold=" +pol+" prhs ="+
            // tyrhs.isPos(ep));
            srec.setCont(rhs);
            srec.setcch(ch);
            srec.setFrame(frame);
