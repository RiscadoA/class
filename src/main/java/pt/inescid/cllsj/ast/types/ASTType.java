...
public abstract class ASTType {
  ...
  public abstract boolean equalst(ASTType t, Env<EnvEntry> e, boolean lit, Trail trail)
      throws Exception;
  ...
  public boolean sameTopType(ASTType t) {
  ...
  public static ASTType unfoldRecInfer(ASTType t, ASTNode here, String ch, Env<EnvEntry> ep)
      throws Exception {
    ...
    if (t instanceof ASTCoAffineT && !done) {
      ...
      if (CLLSj.trace_level == 5) System.out.println("ASTCoAffine unfold infer " + here);
      ...
      {
        if (CLLSj.trace_level == 5) System.out.println("ASTRec unfold infer " + here);
        ASTRecT t0 = (ASTRecT) t;
  ...
  public static ASTType unfoldRecInferParameter(
      ASTType t, ASTType formal, ASTNode here, String ch, Env<EnvEntry> ep) throws Exception {
    ASTNode ancn = here.getanc();
  ...
  public static boolean affine(ASTType t, Env<EnvEntry> ep) throws Exception {
    // USED IN EXPERIMENTAL labeled tensors
    // System.out.println("affine ? "+t);
    if (t instanceof ASTAffineT) return true;
    else if (t instanceof ASTCellT) return true;
    else if (t instanceof ASTStructT) {
      ASTStructT ty = (ASTStructT) t;
      HashMap<String, ASTType> tmap = ty.getcases();
      for (Iterator<String> is = tmap.keySet().iterator(); is.hasNext(); ) {
        String lab = is.next();
