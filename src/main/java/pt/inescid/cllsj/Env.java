...
public class Env<X> {
  ...
  public int level = 0;
  private String id = "$BOT$";
  private volatile X assoc;
  private volatile Env<X> assocs;
  private volatile boolean moved = false; // used for insertWhy
  ...
  public synchronized Env<X> assoc(String id, X assoc) {
    //	if (assoc instanceof SessionClosure) System.out.println("assoc "+id+" "+assoc);
    return new Env<X>(id, assoc, this);
  }

  public synchronized void insert(String _id, X _assoc) {
  ...
  public synchronized Env<X> dupe() {
    // System.out.println("dupe "+this);
    Env<X> de = new Env<X>(id, assoc, assocs, (anc == null) ? null : anc.dup(), moved);
    if (assocs != null) {
  ...
  public synchronized X find(String _id) throws Exception {
    if (assocs != null)
      try { // try overflow list (post-inference of ?x)
        // System.out.println("find overflow "+assocs);
        X elt = assocs.find(_id);
        return elt;
      } catch (Exception e) {
  ...
  public synchronized boolean def(String _id) throws Exception {
    if (_id.equals(id)) {
      return (assoc != null);
    }
    if (anc != null) return anc.def(_id);
    else throw new TypeError(_id + " not declared.");
  }

  public synchronized void upd(String _id, X _assoc) throws Exception {
  ...
  public synchronized void updmove(String _id) throws Exception {
    // System.out.print("updmoves");
    if (_id.equals(id)) {
      moved = true;
      assoc = null;
    } else {
  ...
  public synchronized void updmove(Env<X> env2) throws Exception {
    // System.out.print("updmovee");
    if (moved) {
      env2.upd(id, null);
      moved = false;
    }
  ...
  static synchronized void Pderef(Object assoc) {
