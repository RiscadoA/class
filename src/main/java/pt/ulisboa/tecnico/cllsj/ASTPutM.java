...
public class ASTPutM extends ASTNode {
	...
	if (ty instanceof ASTUsageLT) {
		...
		try {
			...
			throw new TypeError("Line " + lineno + " :" +"PUT " + chs + ": cannot be parsed as put of basic expression nor as free put.");
		}
		lhs.setanc(this);
	    }
	...
	ASTPutM p;
	if (type == null)
	    p = new ASTPutM(chs, label, cho, type, lhs.subst(e), rhs.subst(e));
	else
	    p = new ASTPutM(chs,label, cho, type.subst(e), lhs.subst(e), rhs.subst(e));
	p.rhs.setanc(p);
	p.lhs.setanc(p);
	return p;
    }
