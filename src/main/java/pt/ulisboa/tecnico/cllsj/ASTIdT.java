...
public class ASTIdT extends ASTType {
    ...
    boolean selfte(Env<EnvEntry> e) throws Exception { /* covers only type entries for now */
	EnvEntry ass;
	...
	if (!lit) {
        ...
        // System.out.println(this.toStr(e)+" EQUALS** "+t.toStr(e));
       	...
       	if (this == t || this.equals(t) ) { return true; }
	...
	if (ass instanceof TypeEntry) {
	    ...
	    if (ass instanceof TypeDefEntry) {
		...
		{ Env<ASTType> te = new Env<ASTType>().assoc(par,argi); tdn = tdn.subst(te); }
	    ...
	    // System.out.println("After add pars");
		    ...
		    // ASTType tdn = td.getTypeDef().rhs.unfoldType(en); // note: here we must construct new structure
	    ...
	    throw new KindError(s+"type id is not of the appropriate kind.");
    }
