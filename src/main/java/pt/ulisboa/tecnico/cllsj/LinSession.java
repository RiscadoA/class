...
class LinSession extends Session implements Cell, Channel {
    ...
    */
    ...
    public void setCell(String _chi, ASTNode _rhs, Env<EnvEntry> _ep, Env<LinSession> _ed,
                        Env<Server> _eg, Logger _logger, boolean _linear){
        lock.lock();
        if(fwdSession == null) {
    ...
    public void put(String _chi, ASTNode _rhs, Env<EnvEntry> _ep, Env<LinSession> _ed, Env<Server> _eg) throws Exception{
        lock.lock();
	// Thread.sleep(3);
        while (!cell && fwdSession == null) {
            condition.await();
