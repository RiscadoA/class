...
public class ASTSSelect extends ASTNode{
	...
	if (ty instanceof ASTStructT) {
	    ...
	    if(st==null) 
		throw new TypeError("Line " + lineno + " :" + "STRUCT "+ch
				    +" type mismatch: label "+lab+" not found in "+tys.toStr(ep));
		...
		throw new TypeError("Line " + lineno + " :" + "STRUCT "+cho
				    +" type mismatch: found="+tys_lhs.toStr(ep)+" declared="+typee.toStr(ep));
		...
		try{
			...
			throw new TypeError("Line " + lineno + " :" + "SEND " + ch + ": cannot be parsed as send of basic expression nor as free output.");
		}
		lhs.setanc(this);
	    }
	...
	else throw new TypeError("Line " + lineno + " :" +"STRUCT: "+ch+" is not of STRUCT type, found: "+ty.toStr(ep));
    }
