...
public class ASTCut extends ASTNode {
	...
	// System.out.println("CUT type pol "+cutty.isPos(ep));
	...
	ASTCut p = new ASTCut(id,type.subst(e),lhs.subst(e),
			      rhs.subst(e));
	p.con = con;
	p.lhs.setanc(p);
	p.rhs.setanc(p);
	...
	if (type instanceof ASTintT) {
		// System.out.println("wait_type="+type.toStr(ep));
 ...
 */
    ...
    public void samCCut(Env<SessionField> frame, Env<EnvEntry> ep,
			SAMCont p_cont) throws Exception  {
    ...
    public void samCutV(Env<SessionField> frame, Env<EnvEntry> ep, SAMCont p_cont) throws Exception
    {
			System.out.println("samCutV "+p_cont);
			int i = 0/0;
	}
