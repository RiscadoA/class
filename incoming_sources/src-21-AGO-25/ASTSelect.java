...
public class ASTSelect extends ASTNode {
	...
	if (ty instanceof ASTCaseT) {
	    ...
	    contType = tcase;
	    if(tcase==null) { throw new TypeError("Line " + lineno + " :" +"case: "+lab+" is not a valid label.");}
	    ed.upd(ch,tcase);
	    rhs.typecheck(ed,eg,ep);
	    rhs.linclose(ed,ep);
	} else throw new TypeError("Line " + lineno + " :" +"case: "+ch+" is not of CASE type: found: "+ty.toStr(ep));
    }
