...
public class ASTCoSSelect extends ASTNode {
    ...
    */
	...
	if(_ch.equals(chi)) {
	    ...
	    throw new TypeError("Line " + lineno + " :" +"for " + _ch + " type pending after RECV on "+chr);
	}
	...
	if (ty instanceof ASTCoStructT) {
	    ...
	    //System.out.println("RECV "+chi+" types found="+tyr.getlhs().toStr(ep)+" declared="+typee.toStr(ep));

	    Pair<ASTType,ASTType> st = tyr.step(lab,(coaffine_count>0));

	    if(st==null) 
		throw new TypeError("Line " + lineno + " :" + "COSTRUCT "+chr
				    +" type mismatch: label "+lab+" not found in "+tyr.toStr(ep));
		...
		throw new TypeError("Line " + lineno + " :" +"RECV "+chi+" type mismatch; found="+type.toStr(ep)+" declared="+typee.toStr(ep));
	...
	ASTCoSSelect p;
	if (type == null) 
	    p= new ASTCoSSelect(chi,type,chr,lab,rhs.subst(e));
	else
	    p= new ASTCoSSelect(chi,type.subst(e),chr,lab,rhs.subst(e));
	p.rhs.setanc(p);
	return p;
    }
